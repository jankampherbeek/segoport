...
// Line numbers in original c code. wip = work in progress, ok = finished
// line 90 swe_date_conversion 			ok
// line 113 swe_julday 					ok
// line 181 swe_revjul 					ok
// line 220 swe_utc_time_zone 			ok
// line 273 constants leap seconds 		ok
// line 311 init_leapsec 				ok
// line 356 swe_utc_to_jd 				ok
// line 472 swe_jdet_to_utc
// line 569 swe_jdut1_to_utc

// swe_date_conversion swedate-0113
/*
...
// swe_date_conversion swedate-0090
...
func SweDateConversion(y, m, d int, uttime float64, c rune) (float64, error) {
	gregflag := SE_JUL_CAL
	if c == 'g' {
		gregflag = SE_GREG_CAL
	}
	jd := SweJulday(y, m, d, uttime, gregflag)
	ryear, rmon, rday, _ := SweRevJul(jd, gregflag) // ignore value for uttime (rut in SE)
...
func SweJulday(year, month, day int, hour float64, gregflag int) float64 {
	u := float64(year)
	if month < 3 {
		u -= 1
	}
...
func SweRevJul(jd float64, gregflag int) (int, int, int, float64) {
	u0 := jd + 32082.5
	if gregflag == SE_GREG_CAL {
...
// TODO SweUtcTimeZone might be obsolete.
...
func SweUtcTimeZone(iyear, imonth, iday, ihour, imin int, dsec, dTimezone float64) (int, int, int, int, int, float64) {
	haveLeapsec := false
	if dsec >= 60.0 {
		haveLeapsec = true
		dsec -= 1.0
	...
	iyearOut, imonthOut, idayOut, _ := SweRevJul(tjd+0.001, SE_GREG_CAL)
	ihourOut := int(dhour)
...
//}
//
//func sweDeltatEx(jd float64, iflag int32, serr *string) float64 {
